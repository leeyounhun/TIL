...
- [9. JPA](#9-jpa)
  - [9.1 개요](#91-개요)
  - [9.2 Entity 매핑](#92-entity-매핑)
  - [9.3 영속성 컨텍스트](#93-영속성-컨텍스트)
...
* ORM 기술의 표준 명세로 자바에서 제공하는 API
  * ORM은 서로 패러다임이 다른 객체지향과 관계형 DB 사이에서 2개를 매핑해 주는 작업을 한다.
* 특정 DB에 종속되지 않는다.
* DB 설계 중심의 패러다임에서 객체지향적 설계가 가능하다.
* 생산성이 향상된다
...
* 필드와 컬럼 매핑
  ...
  * 기본 키 매핑
    * @Id를 사용하여 직접 할당
    * @GeneratedValue를 사용하여 자동 생성
* 엔티티 매니저 팩토리
  * 엔티티 매니저를 관리하는 주체
  * 애플리케이션 실행 시 한 개만 만들어 진다.
  * 사용자가 요청하면 엔티티 매니저를 생성한다.
* 엔티티 매니저
  * 엔티티 매니저 팩토리에서 생성되어 DB에 SQL을 전달하며 데이터를 CRUD 한다.
  * 기본적인 메소드
    * persist() : 엔티티를 영속성 컨텍스트에 저장
    * find() :  엔티티를 영속성 컨텍스트에서 검색하고, 없으면 DB에서 데이터를 찾아 영속성 컨텍스트에 저장
...
* 엔티티의 생명주기
  ...
  * 삭제(removed): 삭제된 상태
* 1차 캐시 
  * 영속성 컨텍스트 안에는 1차 캐시가 존재하며 MAP형태로 저장된다.
* 동일성 보장
  * 하나의 트랜잭션에서 같은 키값으로 영속성 컨텍스트에 저장된 엔티티 조회시 같은 엔티티 조회를 보장한다.
  * 1차 캐시에 저장된 엔티티를 조회하기 때문
* 트랜잭션을 지원하는 쓰기 지연
  * 영속성 컨텍스트 안에는 쓰기 지연 SQL 저장소가 존재한다.
  * SQL문을 쌓아뒀다가 트랜잭션을 커밋하는 시점에 저장된 SQL문들이 flush되면서 DB에 반영된다
  * 쿼리를 여러번 날리지 않고 최적화가 가능하다.
* 변경감지
  * 1차 캐시에 DB에서 처음 불러운 엔티티의 스냅샷 값을 가지고 있다.
