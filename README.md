...
* IOC(Inversion Of Control)
  * 제어의 역전
    * 제어는 실행 흐름을 의미
  * 객체의 생성을 특별한 관리 위임 주체에게 맡깁니다. 이 경우 사용자는 객체를 직접 생성하지 않고, 객체의 생명주기를 컨트롤하는 주체는 다른 주체(컨테이너)가 된다.
...
* 자바 객체 - 빈(Bean)들을 관리하는 공간
  ...
  * XML, 어노테이션을 사용하여 관리
    ...
    * 어노테이션은 @configration을 사용하여 관리한다.
      ...
      * 이 방법은 자바 설정 클래스를 이용한다. 
        * 설정 클래스는 @Configuration 어노테이션을 클래스 선언부 앞에 추가하면 된다. 또한 특정 타입을 리턴하는 메서드를 만들고 @Bean 어노테이션을 붙여주면 자동으로 해당 타입의 Bean이 생성된다.
      * 생성자를 통해 Bean의 의존성 주입을 할 수 있다
      * 필드(Property)에 직접 @Autowired 어노테이션을 추가해 의존성을 주입할 수 있다.
      * Setter를 통해 의존성을 주입할 수 있다.
      * @ComponentScan
