...
- [9.1 Join](#91-join)
- [10. JS](#10-js)
  - [10.1 JQuery](#101-jquery)
  - [10.2 Ajax](#102-ajax)
- [11. 추상 클래스와 인터페이스](#11-추상-클래스와-인터페이스)
- [12. 프로세스와 스레드](#12-프로세스와-스레드)
- [13. 공통 프로세스](#13-공통-프로세스)
- [14. HTML](#14-html)
...
## 2.1 개요
  ...
  * 즉 REST란 자원을 이름으로 구분하여 해당 자원의 상태를 주고받는 모든 것,
...
# 9.DB
* DDL: 데이터 정의어, DB에 있는 데이터를 조회, 삽입, 수정, 삭제등을 하기 위한 명령어
* DML: 데이터 관리어, 테이블과 같은 데이터 구조를 정의하는데 사용되는 명령어들로 데이터 구조와 관련된 명령어
* DCL: 데이터 제어어, 데이터베이스에 접근하고 객체들을 사용하도록 권한을 주고 회수하는 명령어\
* 인덱스: 데이터베이스 테이블에 대한 검색 성능의 속도를 높여주는 자료 구조
...
* 둘 이상의 테이블을 연결해서 데이터를 검색하는 방법
  *  테이블들이 적어도 하나의 컬럼을 공유하고 있어야 한다
* 이너 조인: 교집합, 두 테이블이 전부 값을 가지고 있는 컬럼만 생성
* 아우터 조인:  특정 테이블을 기준으로 데이터를 보여준다. 기준이 되는 테이블의 모든 열에 다른 테이블에 있는 공통 영역을 합친 결과를 얻는다.

# 10. JS
## 10.1 JQuery
* JS는 객체 기반의 스크립트 프로그래밍 언어이고 JQuery는 JS기반 라이브러리다.
...
* 웹 페이지 전체를 다시 로딩하지 않고도, 웹 페이지의 일부분만을 갱신할 수 있다.
  ...
  * 서버와는 JSON, XML, HTML, 텍스트 파일등 다양한 형태의 데이터를 주고받는다.
* 클라이언트가 서버에 데이터를 요청하는 클라이언트 풀링 방식을 사용하므로, 서버 푸시 방식의 실시간 서비스는 만들 수 없다.
*  바이너리 데이터를 보내거나 받을 수 없다.

# 11. 추상 클래스와 인터페이스
* 추상 클래스: 클래스 내 추상 메소드가 하나 이상 포함되거나 abstract로 정의된 클래스
* 인터페이스: 모든 메소드가 추상 메소드인 클래스
* 추상 클래스와 인터페이스는 상속받는 클래스 혹은 구현하는 인터페이스 안에 있는 추상 메소드를 구현하도록 강제한다.
* 추상 클래스는 상속-확장, 인터페이스는 구현-규약의 개념이다.
  * 추상 클래스 상속은 슈퍼클래스의 기능을 이용하거나 확장하기 위해서 사용된다.
  * 다중 상속은 모호성 때문에 불가능하다.
  * 인터페이스는 해당 인터페이스를 구현한 객체들에 대해서 동일한 동작을 약속하기 위해 사용된다.
  * 인터페이스는 다중 구현이 가능하다
...
* Filter
  * 요청과 응답을 거른뒤 정제하는 역할
  * 서블릿 필터는 DispatcherServlet 이전에 실행(스프링 외부에 존재)
    * 스프링이 지원하는 기능이 아니다.
    * 필터가 동작하도록 지정된 자원의 앞단에서 요청내용을 변경하거나,  여러가지 체크를 수행
  * 자원의 처리가 끝난 후 응답내용에 대해서도 변경하는 처리를 할 수가 있다.
  * Servlet 단위에서 실행된다
* Interceptor
  * 요청에 대한 작업 전/후로 가로챈다
    * 스프링(Spring)이 제공하는 기술이다.
    * 스프링의 DistpatcherServlet이 컨트롤러를 호출하기 전, 후에 가로챈다
    * 스프링 컨텍스트(Context, 영역) 내부에서 Controller(Handler)에 관한 요청과 응답에 대해 처리
  * Servlet 단위에서 실행된다
  * 스프링의 모든 빈 객체에 접근 가능
  * 여러 개를 사용 가능
* AOP(관점지향)
  * 메소드 앞에 Proxy패턴의 형태로 실행된다.
  * OOP를 보완하기 위해 나온 개념 
* 요청이 들어오면 Filter → Interceptor → AOP → Interceptor → Filter 순으로 거치게 된다.

# 14. HTML
* Hyper Text Markup Language 약어로 HyperText(웹 페이지에서 다른 페이지로 이동할 수 있도록 하는 것) 기능을 가진 문서를 만드는 마크업 언어