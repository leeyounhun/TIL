...
- [7. JQuery](#7-jquery)
  - [7.1 개요](#71-개요)
  - [7.2 사용법](#72-사용법)
  - [7.2.1 선택자(Selecter)](#721-선택자selecter)
...
* 기본 콜백 함수
    * ready() 함수
      * 웹 브라우저가 DOM 트리를 생성한 직후 바로 자동 실행
      * 코드에서 DOM 객체를 사용할 준비 완료
      * 사용자가 이벤트 페이지를 보거나 페이지 상호작용을 하기 전 필요한 작업들 수행
      * $(document).ready(function() {제이쿼리 코드;});
    * load() 함수
      * 메모리에 화면에 필요한 모든 요소를 객체로 생성해서 저장
      * 화면에 필요한 요소(css, js 등)들이 웹 브라우저 메모리에 올려진 후 실행
      * 전체 페이지의 모든 외부 리소스가 전부 로드된 후 작동
        * 외부 리소스를 불러올 때 딜레이가 생기면 그동안 대기해야 한다.
      * body 태그의 onload 이벤트를 사용하면 모든 window.load()가 실행되지 않음
      * $(window).load(function() {제이쿼리 코드;});
      
## 7.2 사용법
* HTML 문서 안의 요소를 찾는다.
  * ID 값으로 찾기 때문에 ID값이 없으면 찾을 수 없다
* 찾은 요소를 조작한다.
